...
typedef std::shared_ptr<FmObjectBase> FmObjectBasePtr;

class ObjectBaseFactory {
public:
    using CreatorFunc = std::function<std::shared_ptr<FmObjectBase>()>;
    ...
    std::shared_ptr<FmObjectBase> CreateObject(const std::string& type) const;
...
inline void ObjectBaseFactory::RegisterType(const std::string& type) {
    creators[type] = []() -> std::shared_ptr<FmObjectBase> {
